setwd("/Users/CDX/R_twitter_politics/labeled_tweet/L")
library(igraph)
library(dplyr)
load("tweet_liberal.RData")# The data list is saved
...
edge_list<-t(sapply(tweet_liberal,FUN = edge))
...
#### make vertices list ####
vertices_list<-t(sapply(tweet_liberal,FUN = function(x) c(x$user$id_str,x$user$name)))
...
vertices_list2<-t(sapply(tweet_liberal,FUN = vertices2))
...
write.csv(vertices_list,file="liberal retweet weight.csv" )
...
png(filename = "image/liberal retweet_graph2.png",width=1400,height=850)
par(mar=c(0,0,0,0))
...
# add title
title("\n liberal tweeters:  Who retweets whom",
